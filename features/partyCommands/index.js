...
const commands = [
	...
	new Command("transfer", ["pt", "ptme"], (player, _, ...args) => {
		if (!args) args = [];
	...
	new Command("kick", ["pt", "ptme"], (player, _, ...args) => {
		if (!args) args = [];
	...
	new Command("promote", [], (player, _, ...args) => {
		if (!args) args = [];
	...
	new Command("demote", [], (player, _, ...args) => {
		if (!args) args = [];
	...
	new Command("allinvite", ["allinv", "ai"], () => {
	...
	new Command("f0", ["f1", "f2", "f3", "f4", "f5", "f6", "f7"], (_, command) => {
	...
	new Command("m1", ["m2", "m3", "m4", "m5", "m6", "m7"], (_, command) => {
...
function execute(player, commandName, ...args) {
	...
	if (command === undefined || !Settings[command.config] || command.permission === "leader" && !leader()) return;
...
const leader = () => Party.leader === Player.getName();
...
function commandListener(command, ...args) {
	switch (command) {
		case "blacklist": {
			chat(blacklist.blacklist.join(", "));
			break;
		}
		case "addblacklist": {
			if (!args[0]) break;
			blacklist.add(args[0]);
			chat(args[0] + " added to blacklist");
			break;
		}
		case "removeblacklist": {
			if (!args[0]) break;
			blacklist.remove(args[0]);
			chat(args[0] + " removed from blacklist");
			break;
		}
		case "clearblacklist": {
			blacklist.clear();
			chat("blacklist cleared");
			break;
		}
		case "whitelist": {
			chat(whitelist.whitelist.join(", "));
			break;
		}
		case "addwhitelist": {
			if (!args[0]) break;
			whitelist.add(args[0]);
			chat(args[0] + " added to whitelist");
			break;
		}
		case "removewhitelist": {
			if (!args[0]) break;
			whitelist.remove(args[0]);
			chat(args[0] + " removed from whitelist");
			break;
		}
		case "clearwhitelist": {
			whitelist.clear();
		...
		default: {
			chat("Party Commands Help:");
			chat("- /sa pc blacklist: List blacklisted players");
			chat("- /sa pc addblacklist <player>: Add a player to the blacklist");
			chat("- /sa pc removeblacklist <player>: Remove a player from the blacklist");
			chat("- /sa pc clearblacklist: Clear the blacklist");
			chat("- /sa pc whitelist: List whitelisted players");
			chat("- /sa pc addwhitelist <player>: Add a player to the whitelist");
			chat("- /sa pc removewhitelist <player>: Remove a player from the whitelist");
			chat("- /sa pc clearwhitelist: Clear the whitelist");
			break;
		}
	}
}
